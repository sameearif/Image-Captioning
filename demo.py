...
if __name__ == "__main__":
    ...
    for f in files:
        ...
        for i in range(data.shape[2]):
            print("Generating captions for", f, i)
            
            pil_image = PIL_Image.fromarray(data[:, :, i])
        ...
        dataframe = pd.DataFrame({"captions": captions, "vectors": vectors})
        dataframe.to_pickle("captions/" + f.split('/')[-1].split('.')[0])
